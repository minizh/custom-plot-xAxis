  ...
  <div class="chart-container">
    ...
    >
      <el-form label-position="top">
        ...
        >
          ...
          <el-form-item label="Orientation" class="flex-item"
            ><el-radio-group v-model="item.orientation" size="small"
              ...
              ><el-radio-button label="tilted"
                >Stanted</el-radio-button
              ></el-radio-group
            ></el-form-item
          >
          <el-form-item
        ...
        <div
          v-for="(_label, index) in yAxisLabels"
          :key="index"
        ...
        <el-form-item
          ><el-radio-group v-model="bgColorMode"
            ...
            ><el-radio label="legend">Legend Color</el-radio></el-radio-group
          ></el-form-item
        >
        <div class="config-section-title" style="margin-top: 24px">
          Color By 配置
        ...
        <template v-if="yAxisLabels.length > 1">
          <div class="colorby-config-row">
            ...
            <el-form-item label="字段" class="flex-item"
              ><el-select
                v-model="colorByDisabledValue"
                disabled
                style="width: 200px"
                ><el-option label="Y-Axis" value="Y-Axis" /></el-select
            ></el-form-item>
          </div>
        </template>
        <template v-else>
...
import {
  useMultiYAxisChart,
  type StatConfig
} from '@/composables/useMultiYAxisChart'
...
const removeStatConfig = (index: number) => {
  if (statConfigs.value.length > 1) statConfigs.value.splice(index, 1)
}
const addYAxisLabel = () => {
  yAxisLabels.value.push(`Y轴${yAxisLabels.value.length + 1}`)
}
const removeYAxisLabel = (index: number) => {
  if (yAxisLabels.value.length > 1) yAxisLabels.value.splice(index, 1)
}
const addColorByConfig = () => {
  colorByConfigs.value.push({ field: colorByFieldOptions[0] || '' })
}
const removeColorByConfig = (index: number) => {
  colorByConfigs.value.splice(index, 1)
}

const { computedYAxisList, computedHeaderLayouts, isColorByMode, getOption } =
  useMultiYAxisChart({
    yAxisLabels,
    rawValues,
    statConfigs,
...
const generateData = () => {
  ...
  for (let i = 1; i <= dataCount.value; i++) {
    ...
    const item: Record<string, unknown> = {
      ...
      testProgram:
        testPrograms[Math.floor(Math.random() * testPrograms.length)],
